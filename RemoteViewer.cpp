...
osg::Camera* RemoteViewer::createMasterCamera()//主控制相机    20151031杜绍敏加的注释
{
...
void RemoteViewer::setMasterCameraLight()//设置主场景的环境光   20151031杜绍敏加的注释
{
	if(mode_node!=NULL)
	{
...
osg::Group* RemoteViewer::scenePrepare()
{
	...
	//sceneGroup->getOrCreateStateSet()->setMode(GL_LIGHTING,osg::StateAttribute::OFF | osg::StateAttribute::OVERRIDE);//这句是杜绍敏201511111138加的
...
{
	//osg::ref_ptr<osg::Node> node=Scene_Mgr::instance()->getSceneByName("cessna");//以飞机模型作为主相机观看的场景的话光照是正常的，前提是在服务器端主场景为飞机，客户端效果还没试
	//osg::ref_ptr<osg::Node> node=Scene_Mgr::instance()->getSceneByName("chair");
	//osg::ref_ptr<osg::Node> node=Scene_Mgr::instance()->getSceneByName("armadillo");//同样加光照的方式，飞机就是正常的，穿山甲就是一片白，待解决
	//osg::ref_ptr<osg::Node> node=Scene_Mgr::instance()->getSceneByName("paris");
	//osg::ref_ptr<osg::Node> node=Scene_Mgr::instance()->getSceneByName("car");
	osg::ref_ptr<osg::Node> node = Scene_Mgr::instance()->getSceneByName("campus");//20140917
...
//	this->getCamera()->addChild(scenePrepare());
//	this->getCamera()->addChild(skynode);
...
void RemoteViewer::frame(double simulationTime)//该函数为仿真循环的最主要函数，用于决定每一帧干什么   20151031杜绍敏加的注释
{
	/*
	统计时间
	*/
	...
	*/
	...
	//updateClientsCamera();//
	TimeForAllClientsPrediction();//结合上一句，先把所有相机是否进行预测的标识置为false，然后判断时间到没到，到了的变成true进行视点预测        20151031杜绍敏加的注释
