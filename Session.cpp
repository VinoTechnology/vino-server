...
Session::Session(boost::asio::io_service& io_service)
	...
	inline void Session::start()
	{
		...
		switch(seq)
		{
		//下面这部分是获取消息的长度，即消息包的大小，供后面的patchmsg函数使用，即提供size变量
		...
		socket_.async_read_some(boost::asio::buffer(data_, size),//async异步处理
			boost::bind(&Session::handle_read, this,
			boost::asio::placeholders::error,
			boost::asio::placeholders::bytes_transferred));
	}
	...
	void Session::patchMsg(Message* msg)
	{
		...
		case CAMERAUP:
			...
			}case NODE_NAME:
			{
				...
				_pipeline->sendMsgToViewMgr(msg);
				cout<<"Node Name"<<endl;
				seq=NODE_NAME;//2014_9_10
				break;
			}
			case NODE_TRANSLATION:
			{
				seq=NODE_TRANSLATION;
				_pipeline->sendMsgToViewMgr(msg);
				cout<<"node translation"<<endl;
				seq=NODE_TRANSLATION;//2014_9_10
				break;
			}
			case NODE_RATATION_AXIS:
			{
				seq=NODE_RATATION_AXIS;
				_pipeline->sendMsgToViewMgr(msg);
				cout<<"node NODE_RATATION_AXIS"<<endl;
				seq=NODE_RATATION_AXIS;//2014_9_10
				break;
			}
			case NODE_ROTATION_DIRECTION:
			{
				seq=NODE_ROTATION_DIRECTION;
				_pipeline->sendMsgToViewMgr(msg);
				cout<<"node NODE_ROTATION_DIRECTION"<<endl;
				seq=NODE_ROTATION_DIRECTION;//2014_9_10;
				break;
			}
		default:
			break;
	...
	Message* Session::translateReceivedDataToMsg(size_t bytes_transferred)
	{
		...
		case INITIAL_SCENE_MSG: {msg=new StringMessage(INITIAL_SCENE_MSG,mylength-12); 
			//msg->setType(INITIAL_SCENE_MSG); //2014_9_10
			//cout<<"消息长度："<<mylength<<endl;
			break;
		}
		case NODE_NAME: msg=new StringMessage(NODE_NAME,mylength-12);break;
		case NODE_TRANSLATION:msg=new NodeTranslationMessage(NODE_TRANSLATION);break;
		case NODE_ROTATION_DIRECTION: msg=new NodeTranslationMessage(NODE_ROTATION_DIRECTION);break;
		case NODE_RATATION_AXIS:msg=new NodeTranslationMessage(NODE_RATATION_AXIS);break;
