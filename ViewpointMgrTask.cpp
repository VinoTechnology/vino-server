 ...
 void  ViewpointMgrTask::start(ServerPipeline* pl)
 {
  ...
  void ViewpointMgrTask::run() {
	 ...
	 while(_runSwitch){
		 ...
		 case INITIAL_RESOLUTION_MSG:
			 {
				 ...
				 res_msg->getResolutionArgs(_client->_resolution);
				 cout<<"分辨率 宽度:"<<_client->_resolution._width<<"高度: "<<_client->_resolution._height<<endl;
				 delete msg;
				 break;
		 ...
		 case NODE_NAME:
			 {
				 StringMessage* sm=dynamic_cast<StringMessage* >(msg);
				 if(_client!=NULL)
					 _client->setMatrixFormNodename(sm->getString());

				 cout<<sm->getString()<<endl;
				 delete msg;
				 break;
			 }
		 case NODE_TRANSLATION:
			 {
				 NodeTranslationMessage* nm=dynamic_cast<NodeTranslationMessage* >(msg);
				 if(_client!=NULL)
				 {
					 _client->setTranslation(nm->getVec());
					 _client->setIsMAtrixForm(true);
				 }
				 cout<<nm->getVec().x()<<endl;
				 cout<<nm->getVec().y()<<endl;
				 cout<<nm->getVec().z()<<endl;
				 delete msg;
				 break;
			 }
		 case NODE_RATATION_AXIS:
			 {
				 //NodeRotation_AxisMessage* nm=dynamic_cast<NodeRotation_AxisMessage*>(msg);
				 NodeTranslationMessage* nm=dynamic_cast<NodeTranslationMessage* >(msg);
				 if(_client!=NULL)
				 {
					 _client->setRotationAxis(nm->getVec());
				 }
				 cout<<nm->getVec().x()<<endl;
				 cout<<nm->getVec().y()<<endl;
				 cout<<nm->getVec().z()<<endl;
		 ...
		 case NODE_ROTATION_DIRECTION:
			 {
				 //NodeRotation_DirectionMessage* nm=dynamic_cast<NodeRotation_DirectionMessage*>(msg);
				 NodeTranslationMessage* nm=dynamic_cast<NodeTranslationMessage* >(msg);
				 if(_client!=NULL)
				 {
					 _client->setRotationDirection(nm->getVec());
					 _client->setisMatrixRotation(true);
				 }
				 cout<<nm->getVec().x()<<endl;
				 cout<<nm->getVec().y()<<endl;
				 cout<<nm->getVec().z()<<endl;
  ...
  void ViewpointMgrTask::initPerspective(Client *client) {
	  ...
	  std::cout << "fov" << client->_pred_perspective._fov << "\t" << client->_perspective._fov << std::endl; 
	  client->_pred_resolution._width = (int)(client->_perspective._aspect *
		  client->_pred_resolution._height);
	  //client->_pred_resolution._width = (int)(1.0f / client->_perspective._aspect *
		//  client->_pred_resolution._height);//9-23 王小川，为AR应用调试修改warping图像分辨率，调换长款顺序
  ...
  void ViewpointMgrTask::initManipulator(Client *client) {//该函数用来根据客户端请求消息中的交互方式来指定客户端的交互方式    20151031杜绍敏加的注释
/*2014_7_7	  
#ifdef RFPM 
	  //12_9
...
*/	
  ...
  void ViewpointMgrTask::initSceneNode(Client *client,string sceneName) {
	  ...
	  if(typeid(*client) == typeid(Client))
	  {
		  ...
		  {
			  Scene_Mgr* scenemgr=Scene_Mgr::instance();
			  osg::MatrixTransform* skynode= new osg::MatrixTransform;
			  skynode->addChild(scenemgr->getSceneByName("sky"));
			  if(scenemgr->getSceneByName("sky")!=NULL)
				  cout<<"sky"<<endl;
			  skynode->setMatrix(osg::Matrix::translate(osg::Vec3(-2000,-2000,-100)));
			  osg::ref_ptr<osg::Group> g=new osg::Group;
			  g->addChild(node);
			  g->addChild(skynode);
			 // g->getOrCreateStateSet()->setMode(GL_LIGHTING,osg::StateAttribute::OFF | osg::StateAttribute::OVERRIDE);//这句是杜绍敏201511111138加的
			  client->createPredictionCamera(g);
		  }else
		  {
			  osg::ref_ptr<osg::Group> g=new osg::Group;
			  g->addChild(node);
	  ...
	  }else if(typeid(*client) == typeid(AppEngineClient))//发动机客户端
	  {
		   osg::ref_ptr<osg::Node> node;
