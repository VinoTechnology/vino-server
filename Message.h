...
#define INVALID_CLIENT_ID 0
	...
	public:
		...
		void getResolutionArgs(Resolution &resolution);
	...
	{
	public:
		StringMessage(int type, size_t s);
	...
	//NODE_RATATION_AXIS,
	//NODE_ROTATION_DIRECTION,
	class NodeTranslationMessage :public Message
	{
	public:
		NodeTranslationMessage(int type);
		size_t serialize(unsigned char *write_buffer);
		size_t deserialize(unsigned char * const read_buffer);
		size_t getSize();
		~NodeTranslationMessage(){};
		osg::Vec3f getVec();
	private:
		osg::Vec3f trans;
	};
