...
_isDied(false),
isMatrixForm(false),
isMatrixRotation(false),
_interval(1000) //2014_10_29
{
...
void Client::setLightBoundingBox(osg::Node* node)
{
	if(node!=NULL)
	{
		...
		for(int i=7;i>=0;i--)
		{
			...
			lt->setAmbient(osg::Vec4(0.5,0.5,0.5,1.0));//ambient应该指的是环境光
			
			lt->setDiffuse(osg::Vec4(1.0,1.0,1.0,1.0));//diffuse应该指漫射光
		...
		//node->getOrCreateStateSet()->setMode(GL_LIGHTING,osg::StateAttribute::OFF | osg::StateAttribute::OVERRIDE);//这句是杜绍敏201511111138加的
	}
...
void Client::createPredictionCamera(osg::Node* node)
{
	...
	_colorCam->setClearColor(osg::Vec4(75.0/256,87.0/256,99.0/256,1.0));
	...
	if(scenemgr->getSceneByName("sky")!=NULL)
		cout<<"sky"<<endl;
	skynode->setMatrix(osg::Matrix::translate(osg::Vec3(0,0,-1000)));
	_colorCam->addChild(skynode);*/
...
//2014_8_16
void Client::setTranslation(osg::Vec3f vec)
{
	this->tran=vec;
}
osg::Vec3f Client::getTranslation()
{
	return this->tran;
}

void Client::setIsMAtrixForm(bool bs)
{
	this->isMatrixForm=bs;
}
bool Client::getIsMatrixForm()
{
	return isMatrixForm;
}

void Client::setRotationAxis(osg::Vec3f vec)
{
	this->axis=vec;
}
osg::Vec3f Client::getAxis()
{
	return axis;
}

void Client::setRotationDirection(osg::Vec3f vec)
{

	this->direction=vec;
}
osg::Vec3f Client::getRotationDirection()
{
	return direction;
}
void Client::setMatrixFormNodename(std::string name)
{
	this->nodeName=name;
}
std::string Client::getMatrixFormNodename()
{
	return nodeName;
}

void Client::setisMatrixRotation(bool bs)
{
	this->isMatrixRotation=bs;
}
bool Client::getisMatrixRotation()
{
	return this->isMatrixRotation;
}
