...
CameraMessage::CameraMessage() {
		...
		StringMessage::StringMessage(int type,size_t s):_strsize(s)
		{
			_msg_type=type;
			_client_id=INVALID_CLIENT_ID;
		}
		...
		void ResolutionMessage::getResolutionArgs(Resolution &resolution){
			resolution = _resolution;
		}

		void ResolutionMessage::setResolutionArgs(const Resolution &resolution) {
			_resolution = resolution;
		}

		size_t ResolutionMessage::getSize(){
			return 12+sizeof(Resolution);
		}

		size_t ResolutionMessage::serialize(unsigned char *write_buffer){
			size_t len = getSize();
			write_uint(write_buffer, 0, &len);
			write_uint(write_buffer, 4, &_msg_type);
			write_uint(write_buffer, 8, &_client_id);
		...
		size_t ResolutionMessage::deserialize(unsigned char *const read_buffer) {
			size_t len = getSize();
		...
		NodeTranslationMessage::NodeTranslationMessage(int type)
		{
			_msg_type=type;
			_client_id=INVALID_CLIENT_ID;

		}
		size_t NodeTranslationMessage::deserialize(unsigned char * const read_buffer)
		{
			float x,y,z;
			read_float(read_buffer,12,&x);
			read_float(read_buffer,16,&y);
			read_float(read_buffer,20,&z);
			trans.set(osg::Vec3f(x,y,z));
			return 29;
		}
		size_t NodeTranslationMessage::serialize(unsigned char *write_buffer)
		{
			//no implement
			return 29;
		}
		size_t NodeTranslationMessage::getSize(){
			return 29;
		}
		osg::Vec3f NodeTranslationMessage::getVec()
		{
			return trans;
		}