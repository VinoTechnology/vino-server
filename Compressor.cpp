...
void Compressor::startJpegCmp(Compressor* cmpor, osg::Image* cSrc)
{
	...
	ofstream outfile1("g://jpeg_time.txt",ofstream::app);
	outfile1<<PROTIME/CNT<<endl;
	outfile1.close();
	ofstream outfile2("g://jpeg_data.txt",ofstream::app);
	outfile2 <<DATASIZE/CNT << endl;
	outfile2.close();
	//IO
	/*std::ifstream colorFile("colorFile.txt",std::ios::in|std::ios::ate);
	colorFile << DATASIZE/CNT << std::endl;*/
...
void Compressor::startDepthCmp(Compressor* cmpor, osg::Image* dSrc)
{
	...
	ofstream outfile3("g://edge_data.txt",ofstream::app);
	outfile3 <<DATASIZE/CNT <<endl;
	outfile3.close();
